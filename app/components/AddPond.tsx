...
const GOOGLE_MAPS_API_KEY = 'AIzaSyA_mFgUqzUKoJytQ5XvKGsSdj-Fw5G8d4g';
...
const AddPond: React.FC<AddPondProps> = ({
    ...
    searchDebounce.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(text)}&key=${GOOGLE_MAPS_API_KEY}&components=country:ph`
        );
  ...
  const handleSelectSuggestion = async (placeId: string, description: string) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(
        `https://maps.googleapis.com/maps/api/place/details/json?place_id=${encodeURIComponent(placeId)}&fields=geometry&key=${GOOGLE_MAPS_API_KEY}`
      );
      const data = await res.json();
      ...
      <Modal visible={locationModalVisible} animationType="slide" transparent={true}>
        <View style={styles.overlay}>
          ...
          <View style={styles.searchContainer}>
            ...
            />
            {searchLoading && <ActivityIndicator size="small" color="#FF8C00" />}
          </View>
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            <Feather name="navigation" size={14} color="#FF8C00" />
          ...
          <Text style={styles.mapHint}>Tap on the map to pin a location</Text>

          {/* Labeled address fields */}
          <Text style={styles.inputLabel}>Street / House No.</Text>
          <TextInput style={styles.input} placeholder="e.g., 123 Main St" value={street} onChangeText={setStreet} />

          <Text style={styles.inputLabel}>Barangay</Text>
          <TextInput style={styles.input} placeholder="e.g., Barangay San Jose" value={barangay} onChangeText={setBarangay} />

          <Text style={styles.inputLabel}>City / Municipality</Text>
          <TextInput style={styles.input} placeholder="e.g., Quezon City" value={cityMunicipality} onChangeText={setCityMunicipality} />

          <Text style={styles.inputLabel}>Province</Text>
          <TextInput style={styles.input} placeholder="e.g., Metro Manila" value={province} onChangeText={setProvince} />
